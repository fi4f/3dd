...
export const scaleColliderArgs = (
  ...
  // HalfSpace uses a vector
  if (shape === "halfspace") {
    const s = newArgs[0] as { x: number; y: number; z: number };
    s.x *= scale.x;
    s.x *= scale.y;
    s.x *= scale.z;
...
export const createColliderFromOptions = (
  ...
  if(options.shape === "halfspace")
    // @ts-ignore
    desc = new ColliderDesc(new HalfSpace(...scaledArgs));
  else 
    // @ts-ignore
    desc = ColliderDesc[options.shape!](...scaledArgs);
